	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for _, row := range req.Rows {
			...
			if row.FolioID == "" && row.DematID == "" {
				results = append(results, map[string]interface{}{"success": false, "error": "Either folio_id or demat_id is required"})
				continue
			}
			...
			if row.ByAmount <= 0 && row.ByUnits <= 0 {
				results = append(results, map[string]interface{}{"success": false, "error": "Either by_amount or by_units must be > 0"})
				continue
			}
...
func UpdateRedemptionBulk(pgxPool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for _, row := range req.Rows {
			if row.RedemptionID == "" {
				results = append(results, map[string]interface{}{"success": false, "error": "redemption_id missing"})
				continue
			}

			tx, err := pgxPool.Begin(ctx)
			if err != nil {
				results = append(results, map[string]interface{}{"success": false, "redemption_id": row.RedemptionID, "error": "tx begin failed: " + err.Error()})
				continue
			}
			defer tx.Rollback(ctx)
			...
			if err := tx.QueryRow(ctx, sel, row.RedemptionID).Scan(
				...
				results = append(results, map[string]interface{}{"success": false, "redemption_id": row.RedemptionID, "error": "fetch failed: " + err.Error()})
				continue
			}
			...
			if len(sets) == 0 {
				results = append(results, map[string]interface{}{"success": false, "redemption_id": row.RedemptionID, "error": "No valid fields"})
				continue
			}
			...
			if _, err := tx.Exec(ctx, q, args...); err != nil {
				results = append(results, map[string]interface{}{"success": false, "redemption_id": row.RedemptionID, "error": "update failed: " + err.Error()})
				continue
			}
			...
			`, row.RedemptionID, row.Reason, userEmail); err != nil {
				results = append(results, map[string]interface{}{"success": false, "redemption_id": row.RedemptionID, "error": "audit insert failed: " + err.Error()})
				continue
			}

			if err := tx.Commit(ctx); err != nil {
				results = append(results, map[string]interface{}{"success": false, "redemption_id": row.RedemptionID, "error": "commit failed: " + err.Error()})
				continue
			}

			results = append(results, map[string]interface{}{"success": true, "redemption_id": row.RedemptionID, "requested": userEmail})
		}

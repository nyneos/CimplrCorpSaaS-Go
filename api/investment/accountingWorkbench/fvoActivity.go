	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		q := `
			...
			SELECT
				...
				fvo.activity_id,
				act.activity_type,
				COALESCE(act.effective_date::text, '') AS effective_date,
				COALESCE(act.accounting_period::text, '') AS accounting_period,
				act.data_source,
				act.status,
				fvo.scheme_id,
				COALESCE(fvo.old_scheme_id,'') AS old_scheme_id,
				COALESCE(fvo.valuation_date::text, '') AS valuation_date,
				COALESCE(fvo.old_valuation_date::text, '') AS old_valuation_date,

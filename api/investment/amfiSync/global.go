	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Body != nil && r.ContentLength > 0 {
			if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
				api.RespondEnvelopeError(w, http.StatusBadRequest, fmt.Sprintf("Invalid request body: %v", err), "")
				return
			}
		}
		...
		if err := pool.QueryRow(ctx, countSQL, filterArgs...).Scan(&total); err != nil {
			api.RespondEnvelopeError(w, http.StatusInternalServerError, fmt.Sprintf("Failed to get count: %v", err), "")
			return
		}
...
LIMIT $%d OFFSET $%d`, where, limitArg, offsetArg)
		...
		if err != nil {
			api.RespondEnvelopeError(w, http.StatusInternalServerError, fmt.Sprintf("Database query failed: %v", err), "")
			return
		}
		defer rows.Close()

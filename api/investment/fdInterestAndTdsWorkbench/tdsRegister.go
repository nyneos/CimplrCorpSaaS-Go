...
func CreateTDSRegister(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		api.RespondEnvelopeSuccess(w, "TDS register entry created successfully", map[string]interface{}{
			"tds_id":              tdsID,
			"fd_id":               req.FDID,
			"fd_ref_no":           fdRefNo,
...
func GetTDSRegisterView(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(map[string]interface{}{ //nolint:errcheck
			...
			"message":    "Success",
			"data":       entries, // primary payload — response.data.data (envelope) matches pre-migration response.data.data (flat)
			"summary":    summary, // kept top-level for back-compat; also see response.data.summary
		})
	}
}
...
func ReconcileTDSRegister(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		api.RespondEnvelopeSuccess(w, "TDS reconciliation completed successfully", map[string]interface{}{
...
func UpdateTDSRegister(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		api.RespondEnvelopeSuccessCompat(w, "TDS entry updated", map[string]interface{}{
			"tds_id": req.TDSID,
		})
	}
}

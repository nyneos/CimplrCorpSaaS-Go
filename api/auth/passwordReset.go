...
func StartTokenCleanup(pool *pgxpool.Pool) {
	go func() {
		for {
			time.Sleep(tokenCleanupPeriod)
			_, _ = pool.Exec(context.Background(), `DELETE FROM password_reset_tokens WHERE (used = true AND created_at < NOW() - INTERVAL '24 hours') OR (expires_at < NOW() - INTERVAL '24 hours')`)
		}
	}()
}
...
// enqueueResetEmail is launched via `go` from a request handler and must outlive
// the request — callers pass context.Background(), never r.Context(), so the
// insert isn't cancelled when the HTTP response finishes.
func enqueueResetEmail(ctx context.Context, pool *pgxpool.Pool, userID, email, name, resetLink string) {
	...
	if err != nil {
		...
		LogSecurityEvent(ctx, pool, "", "reset_email_enqueue_failed", err.Error(), "")
	}
}
...
func ForgotPasswordHandler(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err := pool.QueryRow(r.Context(), `SELECT id, COALESCE(employee_name, '') FROM users WHERE LOWER(email) = $1 AND COALESCE(is_deleted, false) = false LIMIT 1`, email).Scan(&userID, &employeeName)
		if err != nil {
			LogSecurityEvent(r.Context(), pool, "", "forgot_password_unknown", email, clientIP)
		...
		_, _ = pool.Exec(r.Context(), `UPDATE password_reset_tokens SET used = true WHERE user_id = $1 AND used = false`, userID)
		...
		LogSecurityEvent(r.Context(), pool, userID, "forgot_password", email, clientIP)
		...
		// Uses context.Background(), not r.Context() — this goroutine outlives the
		// request and must not be cancelled when the HTTP response finishes.
		go enqueueResetEmail(context.Background(), pool, userID, email, employeeName, resetLink)

		// In dev mode, also include token + link in response for testing
		if strings.EqualFold(os.Getenv("DEVEL_MODE"), "true") {
			authEnvelopeSuccessCompat(w, "Reset token generated (dev mode — email also enqueued)", map[string]interface{}{
				"token":      token,
...
func ResetPasswordHandler(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			LogSecurityEvent(r.Context(), pool, "", "reset_password_invalid_token", "", clientIP)
			writeResetJSON(w, http.StatusBadRequest, false, "Invalid or expired reset token")
			return
		}

		if used {
			LogSecurityEvent(r.Context(), pool, userID, "reset_password_reused_token", "", clientIP)
			writeResetJSON(w, http.StatusBadRequest, false, "This reset token has already been used")
			return
		}

		if time.Now().After(expiresAt) {
			LogSecurityEvent(r.Context(), pool, userID, "reset_password_expired_token", "", clientIP)
		...
		if _, err := tx.Exec(r.Context(), `UPDATE users SET password = $1 WHERE id = $2`, string(hashed), userID); err != nil {
			writeResetJSON(w, http.StatusInternalServerError, false, "Failed to update password")
			return
		}
		if _, err := tx.Exec(r.Context(), `UPDATE password_reset_tokens SET used = true WHERE token = $1`, token); err != nil {
		...
		LogSecurityEvent(r.Context(), pool, userID, "reset_password_success", "", clientIP)
...
func ValidateResetTokenHandler(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			http.Error(w, constants.ErrMethodNotAllowed, http.StatusMethodNotAllowed)
			return
		...
		var email string
		_ = pool.QueryRow(r.Context(), `SELECT email FROM users WHERE id = $1`, userID).Scan(&email)

		authEnvelopeSuccessCompat(w, "Success", map[string]interface{}{
			"valid":      true,
...
func writeResetJSON(w http.ResponseWriter, status int, success bool, message string) {
	if success {
		authEnvelopeSuccess(w, message, nil)
		return
	}
	authEnvelopeError(w, status, message, "")
}

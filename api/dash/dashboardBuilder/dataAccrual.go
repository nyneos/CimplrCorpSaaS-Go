...
func queryFDAccrualRunAll(ctx context.Context, pool *pgxpool.Pool, entityIDs []string, limit int, offset int) ([]map[string]any, error) {
	...
	q := fmt.Sprintf(`
		SELECT
			...
			COALESCE(r.currency_filter, '') AS currency_filter,
			COALESCE(r.accrual_basis, '') AS accrual_basis,
			COALESCE(r.holiday_handling, '') AS holiday_handling,
			COALESCE(r.period_locked, false) AS period_locked,
			COALESCE(r.re_run_allowed, false) AS re_run_allowed,
			r.started_at,
			r.completed_at,
			r.submitted_at,
			COALESCE(r.submitted_by, '') AS submitted_by,
			COALESCE(r.posting_status, '') AS posting_status,
			COALESCE(r.posting_batch_id, '') AS posting_batch_id,
...
func queryFDAccrualScheduleAll(ctx context.Context, pool *pgxpool.Pool, entityIDs []string, limit int, offset int) ([]map[string]any, error) {
	...
	q := fmt.Sprintf(`
		SELECT
			...
			COALESCE(s.accrual_granularity, '') AS accrual_granularity,
			COALESCE(s.period_coverage, '') AS period_coverage,

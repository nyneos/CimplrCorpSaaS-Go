...
func queryFDTDSRegister(ctx context.Context, pool *pgxpool.Pool, entityIDs []string, limit int, offset int) ([]map[string]any, error) {
	args, ef := withEntityFilter(limitOffsetArgs(limit, offset), entityIDs, "t")
	...
	q := fmt.Sprintf(`
		SELECT
			...
			COALESCE(t.tds_rate_expected, 0) AS tds_rate_expected,
			COALESCE(t.tds_expected, 0) AS tds_expected,
			COALESCE(t.tds_variance, 0) AS tds_variance,
			COALESCE(t.bank_tds_reference, '') AS bank_tds_reference,
			COALESCE(t.has_pan, false) AS has_pan,
			COALESCE(t.pan_number, '') AS pan_number,
			...
			COALESCE(t.exception_reason, '') AS exception_reason,
			COALESCE(t.journal_entry_id, '') AS journal_entry_id,
...
func queryFDReceiptAll(ctx context.Context, pool *pgxpool.Pool, entityIDs []string, limit int, offset int) ([]map[string]any, error) {
	...
	q := fmt.Sprintf(`
		SELECT
			...
			COALESCE(r.currency, '') AS currency,
			r.receipt_date,
			r.period_start,
			r.period_end,
...
func queryFDReconcileResults(ctx context.Context, pool *pgxpool.Pool, entityIDs []string, limit int, offset int) ([]map[string]any, error) {
	...
	args, ef := withEntityFilter(limitOffsetArgs(limit, offset), entityIDs, "rc")
...
func queryFDExceptions(ctx context.Context, pool *pgxpool.Pool, entityIDs []string, limit int, offset int) ([]map[string]any, error) {
	...
	q := fmt.Sprintf(`
		SELECT
			...
			COALESCE(ex.case_type,           '') AS case_type,
			COALESCE(ex.result_type,         '') AS result_type,
			COALESCE(ex.receipt_id,          '') AS receipt_id,
			COALESCE(ex.tds_id,              '') AS tds_id,
			COALESCE(ex.proposed_resolution, '') AS proposed_resolution,
			COALESCE(ex.resolution_remarks,  '') AS resolution_remarks,

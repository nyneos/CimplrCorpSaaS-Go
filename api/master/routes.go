...
// RegisterMasterRoutes wires every /master/* route onto mux. Route registration
// only — handler logic lives in the allMasters/investmentMasters/counterpartyHub packages.
func RegisterMasterRoutes(mux *http.ServeMux, serviceName string, pgxPool *pgxpool.Pool) {
	...
	mux.Handle("/master/entity/bulk-create-sync", midGlobalIndep(allMaster.CreateAndSyncEntities(pgxPool)))
	mux.Handle("/master/entity/hierarchy", midGlobalIndep(allMaster.GetEntityHierarchy(pgxPool)))
	mux.Handle("/master/entity/delete", midGlobalIndep(allMaster.DeleteEntity(pgxPool)))
	mux.Handle("/master/entity/findParentAtLevel", midGlobalIndep(allMaster.FindParentAtLevel(pgxPool)))
	mux.Handle("/master/entity/approve", midGlobalIndep(allMaster.ApproveEntity(pgxPool)))
	mux.Handle("/master/entity/reject-bulk", midGlobalIndep(allMaster.RejectEntitiesBulk(pgxPool)))
	mux.Handle("/master/entity/update", midGlobalIndep(allMaster.UpdateEntity(pgxPool)))
	mux.Handle("/master/entity/all-names", midGlobalIndep(allMaster.GetAllEntityNames(pgxPool)))
	mux.Handle("/master/entity/render-vars-hierarchical", midGlobalIndep(allMaster.GetRenderVarsHierarchical(pgxPool)))
	...
	mux.Handle("/master/bank/additional-files/download-main", midGlobalIndep(bankMF.DownloadMain))
	mux.Handle("/master/bank/additional-files/package-zip", midGlobalIndep(bankMF.PackageZip))
	...
	mux.Handle("/master/bankaccount/additional-files/download-main", midCash(bankAccountMF.DownloadMain))
	mux.Handle("/master/bankaccount/additional-files/package-zip", midCash(bankAccountMF.PackageZip))
	...
	mux.Handle("/master/cashflow-category/additional-files/download-main", midCash(cashflowCategoryMF.DownloadMain))
	mux.Handle("/master/cashflow-category/additional-files/package-zip", midCash(cashflowCategoryMF.PackageZip))
	...
	mux.Handle("/master/interest-type/additional-files/download-main", midInvestFD(interestTypeMF.DownloadMain))
	mux.Handle("/master/interest-type/additional-files/package-zip", midInvestFD(interestTypeMF.PackageZip))
	...
	mux.Handle("/master/penalty-structure/additional-files/download-main", midInvestFD(penaltyStructureMF.DownloadMain))
	mux.Handle("/master/penalty-structure/additional-files/package-zip", midInvestFD(penaltyStructureMF.PackageZip))
	...
	mux.Handle("/master/compounding-frequency/additional-files/download-main", midInvestFD(compoundingFrequencyMF.DownloadMain))
	mux.Handle("/master/compounding-frequency/additional-files/package-zip", midInvestFD(compoundingFrequencyMF.PackageZip))
	...
	mux.Handle("/master/tds-plan/additional-files/download-main", midInvestFD(tdsPlanMF.DownloadMain))
	mux.Handle("/master/tds-plan/additional-files/package-zip", midInvestFD(tdsPlanMF.PackageZip))
	...
	mux.Handle("/master/day-count-convention/additional-files/download-main", midInvestFD(dayCountConventionMF.DownloadMain))
	mux.Handle("/master/day-count-convention/additional-files/package-zip", midInvestFD(dayCountConventionMF.PackageZip))
	...
	mux.Handle("/master/bank-config/additional-files/download-main", midInvestFD(bankConfigMF.DownloadMain))
	mux.Handle("/master/bank-config/additional-files/package-zip", midInvestFD(bankConfigMF.PackageZip))
	...
	mux.Handle("/master/bank-rate-card/additional-files/download-main", midInvestFD(bankRateCardMF.DownloadMain))
	mux.Handle("/master/bank-rate-card/additional-files/package-zip", midInvestFD(bankRateCardMF.PackageZip))
	...
	counterpartyHub.RegisterCounterpartyHubRoutes(mux, pgxPool)
}

...
func LookupApprovedEntityID(ctx context.Context, db *pgxpool.Pool, entityName string) (string, error) {
	var id string
	logger.LogInfo("[DEBUG entity_helpers] LookupApprovedEntityID input entityName=%q", entityName)
	err := db.QueryRow(ctx, approvedEntitySQL, entityName).Scan(&id)
	if err != nil {
		logger.LogError("[DEBUG entity_helpers] LookupApprovedEntityID error: %v", err)
		return "", err
	}
	logger.LogInfo("[DEBUG entity_helpers] LookupApprovedEntityID result id=%s for name=%q", id, entityName)
	return id, nil
}
...
func LookupApprovedEntityIDSQLDB(ctx context.Context, db *sql.DB, entityName string) (string, error) {
	var id string
	logger.LogInfo("[DEBUG entity_helpers] LookupApprovedEntityIDSQLDB input entityName=%q", entityName)
	err := db.QueryRowContext(ctx, approvedEntitySQL, entityName).Scan(&id)
	if err != nil {
		logger.LogError("[DEBUG entity_helpers] LookupApprovedEntityIDSQLDB error: %v", err)
		return "", err
	}
	logger.LogInfo("[DEBUG entity_helpers] LookupApprovedEntityIDSQLDB result id=%s for name=%q", id, entityName)
	return id, nil
}

...
func HandleApprove(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			api.LogErrorForResponse(w, "dms rule approve begin: %v", err)
			api.RespondEnvelopeError(w, http.StatusInternalServerError, "failed to approve rules", "DMS_RULE_APPROVE_FAILED")
			return
		}
		...
		if err := tx.Commit(r.Context()); err != nil {
			api.LogErrorForResponse(w, "dms rule approve commit: %v", err)
			api.RespondEnvelopeError(w, http.StatusInternalServerError, "failed to approve rules", "DMS_RULE_APPROVE_FAILED")
			return
		}
		api.RespondEnvelopeSuccess(w, "Rules approved", map[string]interface{}{
			"approved": approved,
			"errors":   errs,
		})
	}
...
func applyApproval(ctx context.Context, tx pgx.Tx, ruleID, actor, ip, checkerComment string) error {
	...
	case "CREATE":
		...
		if _, err := tx.Exec(ctx, `
			UPDATE dms_svc.generation_rule_version SET status = 'APPROVED', approved_by = $1, approved_at = now()
			WHERE version_id = $2::uuid`, actor, *pa.VersionID); err != nil {
			return err
		}
		if _, err := tx.Exec(ctx, `
			UPDATE dms_svc.generation_rule SET processing_status = 'APPROVED', status = 'Active',
				current_version_id = $1::uuid, last_modified_by = $2, last_modified_at = now()
			WHERE rule_id = $3::uuid`, *pa.VersionID, actor, ruleID); err != nil {
			return err
		}
	case "EDIT":
		if _, err := tx.Exec(ctx, `
			UPDATE dms_svc.generation_rule SET
	...
	case "DELETE":
		if _, err := tx.Exec(ctx, `
			UPDATE dms_svc.generation_rule SET is_deleted = true, processing_status = 'APPROVED',
	...
	case "CREATE_VERSION":
		...
		if _, err := tx.Exec(ctx, `
			UPDATE dms_svc.generation_rule SET current_version_id = $1::uuid, processing_status = 'APPROVED',

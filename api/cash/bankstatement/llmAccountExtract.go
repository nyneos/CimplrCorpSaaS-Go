...
func LLMExtractTestHandler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := r.ParseMultipartForm(32 << 20); err != nil {
			fail(http.StatusBadRequest, "failed to parse multipart form: "+err.Error())
			return
		}

		file, _, err := r.FormFile("file")
		if err != nil {
			fail(http.StatusBadRequest, `missing "file" field in form`)
			return
		}
		defer file.Close()
		...
		if err != nil {
			fail(http.StatusInternalServerError, "failed to read uploaded file: "+err.Error())
			return
		}

		rows, err := parseFileToRows(fileBytes)
		if err != nil {
			fail(http.StatusUnprocessableEntity, "failed to parse file (xlsx/xls/csv supported): "+err.Error())
			return
		}

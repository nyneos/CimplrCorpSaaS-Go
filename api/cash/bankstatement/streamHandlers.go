...
// via the CLAUDE.md envelope (api.RespondEnvelopeError).
func respondWithError(w http.ResponseWriter, err error, userMsg string, code int) {
	...
	api.RespondEnvelopeError(w, code, userMsg, "")
}
...
func UploadBankStatementV3Handler(pool *pgxpool.Pool) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if ext == ".pdf" && usePDFCo {
			...
			api.RespondEnvelopeSuccessCompat(w, "Processing your statement. It will appear in Staging shortly.", map[string]interface{}{
				"status":      "accepted",
				"batch_id":    batchID,
		...
		api.RespondEnvelopeSuccessCompat(w, "Success", combinedResponse)
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if stagingID != "" {
			...
			if prevStatus == "committed" && prevBS.Valid && strings.TrimSpace(prevBS.String) != "" {
				api.RespondEnvelopeSuccess(w, "This staged statement was already committed. No changes were applied.", map[string]interface{}{
		...
		if stagingID != "" {
			...
			if st == "committed" {
				...
				api.RespondEnvelopeSuccess(w, "This staged statement was already committed. No changes were applied.", data)
				return
			}
		}
...
func GetPDFMetadataHandler(pool *pgxpool.Pool) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		api.RespondEnvelopeSuccess(w, "Success", row)
	})
}

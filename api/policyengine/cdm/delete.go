...
func HandleDelete(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		rows, err := pool.Query(r.Context(), `
			...
			WHERE variable_id = ANY($1::uuid[]) AND is_deleted = false AND processing_status = 'APPROVED'
			RETURNING variable_id::text`,
			req.IDs, actor)
		if err != nil {
			api.LogErrorForResponse(w, "cdm delete: %v", err)
		...
		for _, id := range affected {
			...
			if err != nil {
				api.LogErrorForResponse(w, "cdm delete audit: %v", err)
			}
		}
		...
		api.RespondEnvelopeSuccess(w, "CDM variable delete requested", map[string]interface{}{
			"requested": affected,
			"skipped":   skipped,
		})
	}

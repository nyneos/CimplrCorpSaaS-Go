...
func insertPolicyChildren(r *http.Request, tx pgx.Tx, policyID string, triggerEvents, modules, subModules, entitiesInclude, entitiesExclude []string, ruleType string, rf ruleFields) error {
	...
	for _, code := range subModules {
		code = strings.TrimSpace(code)
		if code == "" {
			continue
		}
		if _, err := tx.Exec(ctx, `
			INSERT INTO policyengine_svc.policy_sub_module (policy_id, sub_module_code)
			VALUES ($1::uuid, $2)
			ON CONFLICT (policy_id, sub_module_code) DO UPDATE SET is_deleted = false`, policyID, code); err != nil {
			return fmt.Errorf("sub_module %s: %w", code, err)
		}
	}
...
func replacePolicyChildren(ctx context.Context, tx pgx.Tx, r *http.Request, policyID string, triggerEvents, modules, subModules, entitiesInclude, entitiesExclude []string, ruleType string, rf ruleFields) error {
	...
	if _, err := tx.Exec(ctx, `UPDATE policyengine_svc.policy_sub_module SET is_deleted = true WHERE policy_id = $1::uuid`, policyID); err != nil {
		return err
	}

...
func exposureBucketingAdditionalFilesConfig() additionalfiles.Config {
	return additionalfiles.Config{
		...
		Create:        createExposureBucketingAdditionalFile,
...
func createExposureBucketingAdditionalFile(ctx context.Context, tx pgx.Tx, input additionalfiles.CreateInput) error {
	names, err := fxEntityNames(ctx)
	if err != nil {
		return err
	}
	return additionalfiles.InsertAdditionalFileRow(ctx, tx, "public.exposure_bucketing_files", "exposure_header_id", input, `

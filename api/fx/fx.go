...
func NewFXServer(db *sql.DB, port string) (*http.Server, *pgxpool.Pool, error) {
	...
	mux.Handle("/fx/exposures/batch-upload-staging", midFX(exposures.BatchUploadStagingData(db, pgxPool)))
	...
	mux.Handle("/fx/exposures/edit", midFX(exposures.EditExposureHeadersLineItemsJoined(db, pgxPool)))
	...
	mux.Handle("/fx/exposures/delete-multiple-headers", midFX(exposures.DeleteExposureHeaders(db, pgxPool)))
	mux.Handle("/fx/exposures/reject-multiple-headers", midFX(exposures.RejectMultipleExposureHeaders(db, pgxPool)))
	mux.Handle("/fx/exposures/approve-multiple-headers", midFX(exposures.ApproveMultipleExposureHeaders(db, pgxPool)))

	/*bucketing */
	mux.Handle("/fx/exposures/update-bucketing", midFX(exposures.UpdateExposureHeadersLineItemsBucketing(db, pgxPool)))
	mux.Handle("/fx/exposures/get-bucketing", midFX(exposures.GetExposureHeadersLineItemsBucketing(db)))
	mux.Handle("/fx/exposures/bucketing/delete-multiple-headers", midFX(exposures.DeleteBucketingStatus(db, pgxPool)))
	mux.Handle("/fx/exposures/approve-bucketing-status", midFX(exposures.ApproveBucketingStatus(db, pgxPool)))
	mux.Handle("/fx/exposures/reject-bucketing-status", midFX(exposures.RejectBucketingStatus(db, pgxPool)))
	...
	mux.Handle("/fx/exposures/link-exposure-hedge", midFX(exposures.LinkExposureHedge(db, pgxPool)))
	...
	/*-------------     forward    ;)      --------------------*/
	/*mtm upload */
	mux.Handle("/fx/forwards/upload-mtm", midFX(forwards.UploadMTMFiles(db, pgxPool)))
	...
	mux.Handle("/fx/forwards/mtm/update-processing-status", midFX(forwards.BulkUpdateMTMProcessingStatus(db, pgxPool)))
	...
	mux.Handle("/fx/forwards/cancel-roll/action", midFX(forwards.CancellationRolloverAction(db, pgxPool)))

	// Checker (approval) routes
	mux.Handle("/fx/forwards/cancellation-status-request", midFX(forwards.CancellationStatusRequest(db, pgxPool)))
	mux.Handle("/fx/forwards/rollover-status-request", midFX(forwards.RolloverStatusRequest(db, pgxPool)))
	...
	mux.Handle("/fx/forwards/manual-entry", midFX(forwards.AddForwardBookingManualEntry(db, pgxPool)))
	mux.Handle("/fx/forwards/entity-relevant-list", midFX(forwards.GetEntityRelevantForwardBookings(db)))
	mux.Handle("/fx/forwards/update-fields", midFX(forwards.UpdateForwardBookingFields(db, pgxPool)))
	// mux.Handle("/fx/forwards/update-processing-status", midFX(forwards.UpdateForwardBookingProcessingStatus(db)))
	mux.Handle("/fx/forwards/bulk-update-processing-status", midFX(forwards.BulkUpdateForwardBookingProcessingStatus(db, pgxPool)))
	mux.Handle("/fx/forwards/bulk-delete", midFX(forwards.BulkDeleteForwardBookings(db, pgxPool)))
	mux.Handle("/fx/forwards/manual-confirmation-entry", midFX(forwards.AddForwardConfirmationManualEntry(db, pgxPool)))
	mux.Handle("/fx/forwards/upload-multi", midFX(forwards.UploadForwardBookingsMulti(db, pgxPool)))
	mux.Handle("/fx/forwards/upload-confirmations-multi", midFX(forwards.UploadForwardConfirmationsMulti(db, pgxPool)))
...
func StartFXService(db *sql.DB, port string) {
	server, pool, err := NewFXServer(db, port)
	if err != nil {

...
func CancellationStatusRequest(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for bid, amtCancelled := range req.BookingAmounts {
			...
			auditutil.RecordDecision(r.Context(), db, auditutil.DecisionParams{TableName: auditutil.TableForwardCancellation, ParentColumn: "booking_id", ParentID: bid, Status: "APPROVED", CheckerBy: auditutil.Actor(req.UserID), Comment: req.CancellationReason})
...
func RolloverStatusRequest(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for bid, amtCancelled := range req.BookingAmounts {
			...
			auditutil.RecordDecision(r.Context(), db, auditutil.DecisionParams{TableName: auditutil.TableForwardRollover, ParentColumn: "booking_id", ParentID: bid, Status: "APPROVED", CheckerBy: auditutil.Actor(req.UserID), Comment: ""})

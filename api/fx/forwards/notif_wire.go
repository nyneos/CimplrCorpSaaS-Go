...
func fetchBookingIDsByUploadKey(ctx context.Context, db *sql.DB, uploadKey string) []string {
	if strings.TrimSpace(uploadKey) == "" {
		return nil
	}
	rows, err := db.QueryContext(ctx, `SELECT system_transaction_id FROM forward_bookings WHERE upload_s3_key = $1`, uploadKey)
	if err != nil {
		return nil
	}
	defer rows.Close()
...
func fetchBookingIDsByConfirmationUploadKey(ctx context.Context, db *sql.DB, uploadKey string) []string {
	if strings.TrimSpace(uploadKey) == "" {
		return nil
	}
	rows, err := db.QueryContext(ctx, `SELECT system_transaction_id FROM forward_bookings WHERE confirmation_upload_s3_key = $1`, uploadKey)
	if err != nil {
		return nil
	}
	defer rows.Close()
